// src/data/benefits.js
export const BC_BENEFITS = `
    AVAILABLE BC STUDENT BENEFITS:

    1. BC Access Grant
    ID: bc-access-grant
    Category: Student Aid
    Amount: Up to $4,000/yr
    Eligible if: domestic student or permanent resident, income under $100,000/yr, enrolled full-time
    Apply URL: https://studentaidbc.ca
    Apply Label: Apply on studentaidbc.ca

    2. Canada Student Grant
    ID: canada-student-grant
    Category: Student Aid
    Amount: Up to $3,000/yr
    Eligible if: domestic student or permanent resident, financial need, full-time or part-time
    Apply URL: https://www.canada.ca/en/services/benefits/education/student-aid/grants-loans.html
    Apply Label: Apply on canada.ca

    3. GST/HST Credit
    ID: gst-hst-credit
    Category: Tax Credit
    Amount: Up to $519/yr
    Eligible if: filed tax return, income under $50,000/yr, Canadian resident
    Apply URL: https://canada.ca/gst-hst-credit
    Apply Label: Apply on canada.ca

    4. Canada Dental Care Plan
    ID: canada-dental-care
    Category: Health
    Amount: Up to $1,000/yr
    Eligible if: no existing dental insurance, income under $90,000/yr, Canadian resident
    Apply URL: https://www.canada.ca/en/services/benefits/dental/dental-care-plan/apply.html
    Apply Label: Apply on canada.ca

    5. BC Renter's Tax Credit
    ID: bc-renters-tax-credit
    Category: Tax Credit
    Amount: Up to $400/yr
    Eligible if: renting in BC, income under $63,000/yr, filed BC tax return
    Apply URL: https://gov.bc.ca/taxes
    Apply Label: Apply on gov.bc.ca
`;